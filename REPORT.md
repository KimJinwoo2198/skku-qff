...
본 문서는 2-큐비트 PQC를 이용해 고전 논리 게이트를 학습한 일련의 실험 결과를 정리한다. 요구 사항 1~3을 순서대로 다루며, 비얽힘 실험은 `skku1.py`, 얽힘 실험은 `skku2.py` 실행으로 재현할 수 있다.
...
이 문서는 추후 보고서나 발표 자료의 근간으로 사용할 수 있으며, 추가 실험(샷 노이즈 주입, 블록 수 증가, 얽힘 도입 등)을 위한 기준점 역할을 한다.
